...
#
# Usage:
#   tts-audition.sh                           # Audition all installed voices
#   tts-audition.sh --voice en_US-libritts_r-medium --speakers 10   # Random speakers from multi-speaker model
#   tts-audition.sh --voice en_US-libritts_r-medium --range 100-150 # Specific speaker range
#   tts-audition.sh --text "Custom audition line"
#   tts-audition.sh --speed 1.8               # Set speed for all auditions
#
# Options:
#   --voice NAME     Audition speakers within a specific voice model
        ...
        case "$key" in
            ...
            k|K)
                ...
                if [[ -n "$persona_name" ]]; then
                    save_persona "$persona_name" "$voice" "$speaker"
                fi
                return 0
                ;;
            r|R)
